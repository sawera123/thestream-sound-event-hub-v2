// src/pages/Videos.jsx → Supabase Real-time Video Upload with Auto/Manual Thumbnails
...
const Videos = () => {
  ...
  .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'videos' }, async payload => {
    // Fetch the updated row from server to bypass RLS visibility issues
 ...
 // --- Helper: generate auto thumbnail from first frame ---
  ...
  try {
    ...
    if (thumbnailFile) {
      ...
      const { error } = await supabase.storage
        .from('thumbnails')
        .upload(`videos_thumbnail/${thumbnailName}`, thumbnailFile, { upsert: false });
      if (error) throw error;

   } else {
  // Auto-generate thumbnail using working function
  ...
  thumbnailName = fileName;
    ...
    const { data: insertedVideo, error: insertError } = await supabase
      ...
      .insert({
        ...
        category,
        video_url: videoName,
        thumbnail_url: `videos_thumbnail/${thumbnailName}`, // full path saved
        uploaded_by: user.id,
        approved: false
      })
      .select()
